...
if isvector(mu) & size(mu,2)==1 && ~iscell(mu)
  d = length(mu);
  Q = 1; M = 1;
elseif ndims(mu)==2 && ~iscell(mu)
  [d Q] = size(mu);
  M = 1;
elseif iscell(mu)
    Q = numel(mu);
...
if isscalar(Sigma) && ~iscell(Sigma)
  mu = reshape(mu, [d Q*M]);
...
elseif ndims(Sigma)==2  && ~iscell(Sigma)  % tied full
  mu = reshape(mu, [d Q*M]);
...
elseif ndims(Sigma)==3  && ~iscell(Sigma) % tied across M
  B2 = zeros(Q,M,T);
  for j=1:Q
...
if ~iscell(B2)
  ...
  for q=1:Q
    %B(q,:) = mixmat(q,:) * squeeze(B2(q,:,:)); % squeeze chnages order if M=1
    B(q,:) = mixmat{q} * B2{q}(:,:); % vector * matrix sums over m
  end
end
%t=toc;fprintf('%5.3f\n', t)
