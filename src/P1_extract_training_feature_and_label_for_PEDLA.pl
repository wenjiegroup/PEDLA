...
###<discription>this program extracts features and labels for PEDLA according to the training set. Each enhancher or non-enhancher in the training set is treated as a variable-length sequence that will be used to train PEDLA.

###input features of the program are in sub-directories of the directory "RPKM/Data_matrix_combined-step200win200" and input labels are in sub-directories of the directory "Training_set/Training_set9"
###output of the program is in the corresponding sub-directories of the directory "Data_for_training_and_prediction/Data_for_training_and_prediction-ratio9step200win200". In the output directory there will be about 2*N files, where N is the total number of ehnhancers and non-enhanchers in the training set. The N files prefixed by "x_" are the features and the other N files prefixed by "y_" are corresponding labels.
...
foreach my $cell(sort keys %cell_with_marker)
{
	...
	$dir_out.="/$dir_out-ratio${num_negative_set}step${bin_size}win$win";
	mkdir $dir_out unless -e $dir_out;
	$dir_out.="/${cell}";
	mkdir $dir_out unless -e $dir_out;
