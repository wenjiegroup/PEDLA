...
###<discription>this program extracts 1114-D unlabeled features of the whole genome of H1 for PEDLA to predict. 

###input features of the program are in sub-directories of the directory "RPKM/Data_matrix_combined-step200win200.H1"
###output of the program is in the corresponding sub-directories of the directory "Data_for_training_and_prediction.H1/unlabelled_data-step200win200". In the output directory there will be about 23 files, where each corresponds to a chromosome. The 23 files prefixed by "x_" are the features.
...
foreach my $cell(sort keys %cell_with_marker)
{
	...
	my $dir_out="Data_for_training_and_prediction.H1";
